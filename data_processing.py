...
def readEvents(csv_path, cols=[]):
    
    """Reads the event info from a txt file (CSV)
    Parameters
    ----------
    csv_path : the path to the event information file

    Returns
    -------
    events : numpy matrix with event information

    Examples
    --------
    >>> csv_path = "/PATH/TO/CSVFILE/events.csv"
    >>> events = readEvents(csv_path)
    """
    
    if cols == []:
        events = np.loadtxt(open(csv_path,"rb"))
...
def saveToTxt(data, path):
    ...
    Parameters
    ----------
    data: numpy matrix to be saved onto file
