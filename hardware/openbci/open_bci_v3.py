...
FIXME: at the moment we can just force daisy mode, do not check that the
module is detected.
...
class OpenBCIBoard(object):
    ...
    def start_streaming(self, callback, lapse=-1):
        ...
        (every two samples with daisy module).
        ...
        while self.streaming:
            ...
            if self.daisy:
                ...
                # even sample: concatenate and send if last sample was the fist
                # part, otherwise drop the packet
                elif sample.id - 1 == self.last_odd_sample.id:
                    # the aux data will be the average between the two samples,
                    # as the channel samples themselves have been averaged by
                    # the board
                    avg_aux_data = list(
                        (np.array(sample.aux_data)
                         + np.array(self.last_odd_sample.aux_data)) / 2)
                    whole_sample = OpenBCISample(
                        sample.id, sample.channel_data
        ...
        for rep in range(max_bytes_to_skip):
            ...
            #---------Channel Data---------
                ...
                for c in range(self.eeg_channels_per_sample):
                    ...
                    # unpack little endian(>) signed integer(i) (makes
                    # unpacking platform independent)
